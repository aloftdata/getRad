get_pvol_ro <- function(radar, time, ..., call = rlang::caller_env()) {
  ...
  urls <- glue::glue(
    getOption(
      "getRad.ro_url",
      default = "https://opendata.meteoromania.ro/radar/{toupper(substr(radar,3,5))}/{toupper(substr(radar,3,5))}_{strftime(time,'%Y%m%d%H%M', tz='UTC')}0200{params}.hdf"
    )
  )
  tryCatch(
    ...
    error = function(cnd) {
      urls_updated <- glue::glue(
        getOption(
          "getRad.ro_url_alt",
          default = "https://opendata.meteoromania.ro/radar/{toupper(substr(radar,3,5))}/{toupper(substr(radar,3,5))}_{strftime(time,'%Y%m%d%H%M', tz='UTC')}0300{params}.hdf"
        )
      )
      if (
