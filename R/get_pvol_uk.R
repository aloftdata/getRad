get_pvol_uk <- function(radar, time, pulse_type=c("lp", "sp"), ..., call = rlang::caller_env()) {
  ...
  if(pulse_type=="sp" && time !=lubridate::floor_date(time,"10 mins")){
    cli::cli_abort(call=call, c(x="Short pulse data is only available every ten minutes.", i="To resolve round the {.arg time} to the nearest 10 minutes."), class="getRad_error_uk_no_sp_data")
  }
  withr::with_file("file.h5", {
    ...
    req <- tryCatch(httr2::request(url) |>
      req_user_agent_getrad() |>
      httr2::req_perform(path = "file.h5", error_call = call),
    httr2_http_404 = function(cnd) {
      cli::cli_abort(c(x="No data was found for this radar ({.val {radar}}) at the specified search time ({.val {time}})", i="The url constructed was: {.url {url}}."),
        call = call,
        cnd = cnd,
