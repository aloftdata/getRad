...
#' @returns `set_secret()` returns `TRUE` when a secret has successfully been
  ...
  if (!(sname %in% keyring::key_list(sname)$service)) {
    cli::cli_abort(
      c(
        ...
        "i" = "Please use {.run getRad::set_secret(\"{name}\")} to store the secret. Note that the prefix is automatically added in {.fun set_secret}."
      ),
