test_that("Pvol for uk can be downloaded", {
  ...
  pvolsp <- expect_s3_class(get_pvol("ukjer", pulse_type="sp", time, param = "all"), "pvol")
...
test_that("404 no data",{
  skip_if_offline("ncas-radar-o.s3-ext.jc.rl.ac.uk")
  expect_error(get_pvol("ukcas", as.POSIXct("2012-4-1 10:00:10")),class="getRad_error_uk_no_data_404")
})
...
test_that("no sp data",{
  ...
  expect_error(get_pvol("ukcas", time, pulse_type="sp"),class="getRad_error_uk_no_sp_data")
})
