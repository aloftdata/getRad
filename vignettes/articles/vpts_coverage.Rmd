...
for (i in unique(cvr$source)) {
  ...
  if (any(is.na(res$longitude))) {
    ll <- p(glue::glue("One or more radars were omited from the map as no location is found in the opera archive ({glue::glue_collapse(res$radar[is.na(res$longitude)], ', ', last = ' and ' )})."))
    }
