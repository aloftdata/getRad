...
When using these datasets ensure that you reference them appropriately as well as the R package you used to process them (e.g. `citation("getRad")` and `citation("bioRad")`)
...
#  res$grph[[1]]
  ...
  html <- c(html, list(h3("Data source:",i), 
                       lft, ll, 
                       h4("Data reference"),
                      HTML( format(getRad:::vptsReferences[i],"html"))
                       ))
}
tagList(html)
```
...
Data can also be read from a local copy of a repository. This can be either data formatted like it is on `aloft` or from `dark_ecology`. In the former case the appropriate references are mentioned above. For `dark_ecology` data reference the following article:

```{r de}
tagList(list(HTML( format(getRad:::vptsReferences["dark_ecology"],"html"))))
```
