...
opera<-dplyr::tribble(
  ~code, ~description, ~time, ~supported, ~license, ~links,
    "opera", "Data is publicly available", "Data is available for the last 24 hours","Readable","CC BY 4.0<sup>    <a href='https://api.meteogate.eu/eu-eumetnet-weather-radar/docs'>1</a> </sup>",c("Information"="https://meteogate.eu/"),
...
included_countries <- dplyr::tribble(
  ...
  dplyr::left_join(dplyr::bind_rows(   rnaturalearth::ne_countries("medium") |>
      dplyr::mutate(key_lower = tolower(iso_a2_eh)) |> dplyr::filter(key_lower %in%c("be", "ch", "cz", "de", "dk", "es", "ee", "fi", "fr", "hr", #"ie", 
"is", "lt", "mt", "nl", "no", "pl", "ro", "se", "si")) |> dplyr::summarise(key_lower="opera", name="Europe"),
